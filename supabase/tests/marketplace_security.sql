...
BEGIN
  ...
  SELECT count(*) INTO n FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace s ON s.oid=c.relnamespace JOIN pg_proc f ON f.oid=t.tgfoid
  WHERE s.nspname='public' AND c.relname='profiles' AND f.proname='protect_profile_role' AND NOT t.tgisinternal;
  IF n <> 1 THEN RAISE EXCEPTION 'Profile role protection trigger is missing'; END IF;
...
WHERE n.nspname='public' AND c.relname IN ('vendors_public','riders_public');
