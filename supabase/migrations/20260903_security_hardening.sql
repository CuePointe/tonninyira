...
create table if not exists public.profiles (
  ...
  display_name text, phone text, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
drop policy if exists "riders_staff_read" on public.riders;
drop policy if exists "riders_staff_update" on public.riders;
...
-- Prevent a vendor/customer from rewriting financial, ownership, or identity fields during updates.
...
begin
  if auth.uid() is not null and not public.is_staff_or_admin() then
    if new.user_id is distinct from old.user_id or new.vendor_id is distinct from old.vendor_id
       or new.items is distinct from old.items or new.total is distinct from old.total
       or new.subtotal is distinct from old.subtotal or new.item_subtotal is distinct from old.item_subtotal
       or new.delivery_fee is distinct from old.delivery_fee or new.customer_phone is distinct from old.customer_phone
    ...
    if exists(select 1 from public.vendors v where v.tonninyira_id=old.vendor_id and v.auth_user_id=auth.uid()) then
      if new.status not in ('accepted','declined','ready','completed') then raise exception 'Invalid vendor status'; end if;
...
create trigger protect_order_fields before update on public.orders for each row execute function public.protect_order_fields();

-- Public-facing views intentionally exclude phone, PIN hashes, NID and compliance data.
create or replace view public.vendors_public with (security_invoker=true) as
select tonninyira_id,business_name,location,category,subcategory,logo_url,gallery,items,created_at from public.vendors where approval_status='approved';
create or replace view public.riders_public with (security_invoker=true) as
select tonninyira_id,full_name,photo_url,vehicle_type,plate_number from public.riders where approval_status='approved';
